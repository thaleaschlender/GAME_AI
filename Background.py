...
class Background():
    ...
    def __init__(self, DISPLAYSURF):
        ...
        self.cloud_pixels = self.generate_fractal_noise_2d(shape=(self.rectBGimg.width + 188, self.rectBGimg.height + 388),
                                                      res=(5,5), octaves=3, persistence=0.5)
        for kx in range(self.rectBGimg.width):
            for ky in range(self.rectBGimg.height):
                ...
                self.bgimage.set_at((kx, ky), (np.min([int(c * 255),255]), np.min([col[1] + (int(c * 255)),255]), np.min([col[2] + (int(c * 255)),255])))
